...
void Object(_render_triangle)(SHAPE *_shape, Shader *_shader) {
    ...
    glm_translate(_transform, (vec3){_shape->_object._position[0], _shape->_object._position[1], _shape->_object._position[2]});
    
    glm_translate(_transform, (vec3){0.5f * _shape->_object._scale[0], 0.5f *_shape->_object._scale[1], 0.0f});
    glm_rotate(_transform, _shape->_object._rotation[0], (vec3){1.0f, 0.0f, 0.0f});
    glm_rotate(_transform, _shape->_object._rotation[1], (vec3){0.0f, 1.0f, 0.0f});
    glm_rotate(_transform, _shape->_object._rotation[2], (vec3){0.0f, 0.0f, 1.0f});
    glm_translate(_transform, (vec3){-0.5f * _shape->_object._scale[0], -0.5f *_shape->_object._scale[1], 0.0f});
    
    glm_scale(_transform, (vec3){_shape->_object._scale[0], _shape->_object._scale[1], 1.0f});
    
    glBindTexture(GL_TEXTURE_2D, 0);
    
    Molson(_set_vector3_f)("_colour", _shape->_object._colour, false, _shader);
    Molson(_set_matrix4)("_transform", &_transform, false, _shader);
    Molson(_set_bool)("_is_textured", false, _shader);
    
    glBindVertexArray(_shape->_object._vao);
...
SHAPE Object(_new_triangle)(const char *_name, vec2 _initial_position, vec2 _initial_scale, vec3 _initial_rotation, vec3 _initial_colour) {
    ...
    _triangle._object._position[0] = _initial_position[0];
    _triangle._object._position[1] = _initial_position[1];
    
    _triangle._object._colour[0]   = _initial_colour[0];
    _triangle._object._colour[1]   = _initial_colour[1];
    _triangle._object._colour[2]   = _initial_colour[2];
    
    _triangle._object._scale[0]    = _initial_scale[0];
    _triangle._object._scale[1]    = _initial_scale[1];
    
    _triangle._object._rotation[0]    = _initial_rotation[0];
    _triangle._object._rotation[1]    = _initial_rotation[1];
    _triangle._object._rotation[2]    = _initial_rotation[2];
    
    _triangle._initialized = 1;
    _triangle._shape_type = "TRIANGLE";
    printf("[INFO] Triangle '%s' had been created. \n", _triangle._name);
    return _triangle;
}
